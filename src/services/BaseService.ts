...
export class BaseService {
    ...
    constructor(entityManagerOrRunner?: EntityManager | QueryRunner) {
        if (!entityManagerOrRunner) {
            ...
            if (entityManagerOrRunner.isTransactionActive) {
                this.transaction.use(entityManagerOrRunner);
            }
        } else {
    ...
    public get connection(): EntityManager {
    ...
    /**
     * When connection is assigned, it will be assigned to all repositories
     * marked with the @Repository decorator
     * @param connection
     */
    public set connection(connection: EntityManager) {
        this._connection = connection;
        this.assignManagerToRepositories(connection);
    }

    private set queryRunner(runner: QueryRunner | undefined) {
        this._queryRunner = runner;
    }

    private get queryRunner(): QueryRunner | undefined {
    ...
    transaction = {
        ...
        run: async <T>(
            ...
            const queryRunner = await this.transaction.start();
            try {
        ...
        start: async (): Promise<QueryRunner> => {
        ...
        commit: async () => {
            if (
                this.transaction.isOwner && // Only owner can commit
                this.transaction.isTransactionActive &&
                this.queryRunner
            ) {
            ...
            startSkipped: () => {
                this.log(
                    `${this.constructor.name}: Using existing transaction from ${
                        (this.queryRunner?.manager as TraceableEntityManager)?.owner
                    }`
                );
            },
            commitSkipped: () => {
